...
void CyclogramConsole::onCommandStarted(Command* command)
{
    ...
    addMessage(QColor::fromRgba(color), QString("Command '%1' started").arg(command->text()));
}
...
void CyclogramConsole::onCommandFinished(Command* command)
{
    ...
    addMessage(QColor::fromRgba(color), QString("Command '%1' finished").arg(command->text()));
}
