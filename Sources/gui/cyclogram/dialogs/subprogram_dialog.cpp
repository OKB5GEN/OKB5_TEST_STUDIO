...
/* Рубрика "Мысли вслух"
 *
 * Описание ситуации: Юзер создает циклограмму, создает в ней подпрограммы, в подпрограммах еще подпрограммы, прописывает маппинги.
 * После этого он закрывает приложение/хочет открыть другой файл/хочет сделать новый файл. Нужно все открытое закрыть, что надо сохранить.
 *
 * Проблемы:
 * 1. Если не соблюсти правильный порядок закрытия окон, то можно потерять изменения. Закрывать надо от дальних к ближним (от самой глубокой подпрограммы к главной циклограмме)
 * 2. Если окон подпрограмм много, то надо полчаса вводить имена файлов, под которым сохранять.
 * 3. ВОЗМОЖНО: В главной циклограмме А сделали подпрограмму Б с переменной Х. В подпрограмме Б сделали подпрограмму В с переменной У.
 * А подпрограмме Б прописали, что после выполнения подпрограммы В в переменную Х надо записать значение переменной У. После этого закрываем окно подпрограммы Б.
 * В подпрограмме В удаляем переменную У. Подпрограмма Б по идее должна сломаться (из-за того, что переменная Х не обновится когда надо, и вообще интересно как загрузится подпрограмма Б
 * по идее при загрузке маппинга команды подпрограммы в Б надо проверять входной и выходной маппинг на предмет существующих переменных подпрограммы, если переменной не стало - выдавать как минимум ворнинг
 * хотя бы в выходном маппинге, ибо во входном маппинге удаление переменной не критично для Б)
 * 4. По-прежнему стоит проблема редактирования одной и той же циклограммы (разные команды-подпрограммы) в разных окнах (надо обновлять или что-то делать понятное для юзера)
 * Вероятно QFileSystemWatcher юзать для обновления файла циклограммы
 *
 * Хотелки:
 * 1. Изменения не должны профакиваться
 * 2. При закрытии главной циклограммы и всех окон показывать только Save All окно, а не стопицот по отдельности с выбором имени файла
 * 3. Имя файла подпрограммы должно генерироваться сразу при создании команды-подпрограммы
 *
 * Варианты решения:
*/
