...
CmdSelectState::CmdSelectState(QObject* parent):
    CmdQuestion(DRAKON::SELECT_STATE, 3, parent)//,
//    mOperation(Equal),
...
void CmdSelectState::insertCommand(Command* newCmd, ValencyPoint::Role role)
{
    insertInSwitchState(newCmd, role);
}

void CmdSelectState::insertInSwitchState(Command* newCmd, ValencyPoint::Role role)
{
...
void CmdSelectState::insertCycleToSwitchState(Command* newCmd, ValencyPoint::Role role)
{
    int TODO; // need check
...
void CmdSelectState::insertIfToSwitchState(Command* newCmd, ValencyPoint::Role role)
{
    if (role == ValencyPoint::UnderArrow)
    {
...
void CmdSelectState::insertSwitchStateToSwitchState(Command* newCmd, ValencyPoint::Role role)
{
    if (role == ValencyPoint::UnderArrow)
    {
        LOG_WARNING(QString("Unexpected valency point"));
