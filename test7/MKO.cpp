...
void MKO::startMKO1()
{
    ...
    if (!hEvent1) { data+="Ошибка! CreateEvent() не запустился!\n";
        printf("CreateEvent() failed!\n");   }
    if (!hEvent) { data+="Ошибка! CreateEvent() не запустился!\n";
        printf("CreateEvent() failed!\n");   }
    if (TmkOpen()) {  data+="Ошибка! Библиотека МКО не запустилась!\n";
        printf("TmkOpen() failed!\n");      }
    else printf("TmkOpen() successful!\n");
    if (tmkconfig(1) != 0 ){ data+="Ошибка! Конфигурация МКО 1 не произошла!\n"; }
    else  printf("tmkconfig1() successful!\n");
    if (tmkconfig(0) != 0 ){ data+="Ошибка! Конфигурация МКО 0 не произошла!\n"; }
    else  printf("tmkconfig0() successful!\n");
    if (bcreset()!=0 ){ data+="Ошибка! Перезагрузка МКО не произошла!\n"; }
...
void MKO::startMKO()
{
    ...
    if (tmkconfig(0) != 0 ){ data+="Ошибка! Конфигурация МКО не произошла!\n"; }
...
void MKO::MKO_start_test(int x, int adr1, int adr2)
{
    ...
    if (x==1 || x==2){
        ...
        bcgetblk(0,buff,12);
        for(int i=1;i<12;i++){
            if(buff[i]!=0)err+="Ошибка! Тест провален!";
        }
        err+=OCcontrol(buff[0]);
    }
    if(x==3){
        ...
        ch_cur=3;
        if (tmkselect(0)!= 0 ){ err+="Ошибка! tmk0!\n"; }//
        bcreset();
        ...
        if (tmkselect(1)!= 0 ){ err+="Ошибка! tmk1!\n"; }//
        bcreset();
...
void MKO::MKO_tr_cm(int x,QString cm, int adr1, int adr2)
{
    ...
    if(x==3){
        ...
        ch_cur=2;*/
        //bcreset();
        if (tmkselect(1)!= 0 ){ err2+="Ошибка! tmk1!\n"; }//
        bcreset();
        AddrOU = adr2;
...
void MKO::MKO_rc_cm(int x, int adr1, int adr2)
{
    ...
    if (x==3){
        ...
        ch_cur=3;
        if (tmkselect(1)!= 0 ){ err3+="Ошибка! tmk0!\n"; }//
        bcreset();
        Sleep(100);
        SubAddrOU = 13;
        msgCmd = (AddrOU<<11)+RT_TRANSMIT+(SubAddrOU<<5)+(21 & NWORDS_MASK);
        ...
        if (tmkselect(0)!= 0 ){ err3+="Ошибка! tmk1!\n"; }//
        bcreset();
        Sleep(100);
        err3+=" Резервный полукомплект прием массива: \n";
        AddrOU = adr2;
